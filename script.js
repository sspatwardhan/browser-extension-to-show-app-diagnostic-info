...
async function createTcsApiToken(){
  ...
  const res = await fetch(`${baseURL}/v1/api/application`, {
    ...
    body: "{\"appName\":\"CS Diagnostics - Browser Extension\",\"description\":\"Autogenerated by browser extension\"}",
    method: "POST",
  });
  creteTokenResponse = await res.json();
  setItUp('CSApiToken',`API Token: ${creteTokenResponse.appToken}`,'API Token: ')
}
  ...
  if ( requestedFlagName == ''){
    ...
    if(resJson[requestedFlagName] == undefined){
      ...
      featureFlagButton.addEventListener("click", function() {
        ...
        const targetURL = `https://app.launchdarkly.com/orgName/${tcsContainerSite.replace("-","_")}/features/${requestedFlagName}/targeting`;
